...
// 인스턴스의 프로퍼티를
// prototype으로 만든 인스턴스 프로퍼티 보다
// 먼저 사용한다.
// 위에 구조를 봐도 프로토타입에 있는 getPoint는
// 한 단계 들여쓰기가 되어 있는 것을 확인할 수 있다.

// 인스턴스마다 값을 다르게 가질 수 있음!
// 이것이 인스턴스를 사용하는 중요한 목적이다.
// objA.point, objB.point, objC.point처럼
// 각 인스턴스마다 다른 값을 가질 수 있다.

// 하지만 이와 다르게
// [[Prototype]]에 있는 것들은 똑같다.
// 이게 인스턴스의 특징!!
...
// 위에서 getPoint가 2개가 있다.
// 하나는 [[Prototype]]에 설정된 것,
// 다른 하나는 obj인스턴스에 바로 설정된 것.
...
// ES5에는 class를 지원해주지 않지만
// 위의 형태는 class형태이다.

// 그럼 왜 굳이 이렇게 인스턴스로 사용하면 되지
// Class를 만드냐는 것이다.
// 인스턴스는 메소드와 프로퍼티의 묶음이다.
// 이렇게 하나로 묶는 이유는
// 데이터 중심으로 접근하겠다는 것이다.

// 이는 곧 인스턴스마다 값을 가져가는 것이고,
// 이는 즉! 데이터만 다르게 가고
// 메소드는 같은 것을 공유하겠다는 것이다.

// 함수를 만약에 인스턴스처럼 사용하기 위해서는
// 이 함수를 저장할 별도의 장치가 필요한데,
// 인스턴스는 그럴 필요 없이 값을
// 서로 다르게 가져갈 수 있다.

// 이렇게 Class의 관점에서 접근을 하려면
// 설계가 중요하다. 설계를 위해서는 OOP개념 이해가 필요하다.
// JS는 OOP언어이다.
// 시스템이 복잡해지면 객체지향으로 갈 수 밖에 없다.
