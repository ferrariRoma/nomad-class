...
// 그래서 키워드는 context이다. 얼마나 간단한 context를 만들 것이냐?
...
// 시맨틱을 고려해서 변수의 이름을 작성하다 보면 겹치는게 꼭 있다.
// 그래서 스코프가 필요하다.
...
// 함수가 실행될 때 실행된 결과를
// 그리고 실행하기 위한 환경을 만든다.
// => 역할만 보면 꼭 5에서 실행 콘텍스트의 역할인 것 같다.
...
// 3:37, 5:1, 6:0
// 5에서 한 번 사용되었는데
// 이마저도 바뀐 것을 나타내기 위해 사용
// scope chain형태를 아래와 같이 바꿈.
// lexical environment의
// declarative environment record(DER)에
// 함수의 변수와 함수 이름을 바인드 하였음.
// 3에서는 scope chain에 바인딩 하였는데 5에서는 DER로 바뀜.
// 함수의 변수와 이름을 선언적 환경 레코드에 바인딩 한다는 것이다.

// 3은 스코프 체인이라는 구조체를 거쳐서 식별자 해결이 진행되는 것이고
// 5는 그냥 바로 DER에서 찾으면 되는거니 거쳐야 하는 추가적인 단계가 없는 것이다.
// 유일한 콘텍스트를 만들 수 있다.
...
/* show 실행 콘텍스트(EC):{
  렉시컬 환경 컴포넌트(LEC):{
    환경 레코드(ER):{
      선언적 환경 레코드(DER):{=>함수가 호출됐을때 내부의 변수와 함수가 있는곳(예전 스코프 체인 담당)
      },
      오브젝트 환경 레코드(OER):{}
    },
...
// 서버 프로그램 고려 사항으로는
// 동점 10k이다.
// 만명이 동시 접속 했을 때도 멀쩡히 돌아가는 지를 고려하자!

// JS는 싱글 스레드이다.
// 싱글 스레드라는 것인 하나의 함수가 호출되어서 끝날때까지
// 다른 함수를 호출하지 않는 것이다.
// 따라서 순차적으로 처리한다.
// 이를 동기처리라고 한다.
...
// 아이딜 타임이 생기면 웨이트 큐(wait queue)에 있는 프로세스를 실행시킨다.
// 즉, 비동기 처리를 하는 것이다.
...
// ES5 엔진 처리 중심
// ES5 스펙의 모든 항목을 빠짐없이 다 다루고
// 각 항목의 바탕이 되는 개념과 파생되는 활용을 다룬다.
// 기눙 보다는 논리에 중점을 두고 접근할 것이다.

// 결과를 보고 '아 이렇게 되는거야?'
// 하는게 아니라 '어떻게 해서 그렇게 되는거야?'로 접근 할 것이다.
